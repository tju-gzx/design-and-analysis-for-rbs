...
def select(popular_gene, select_score):
    ...
    # 提取前50个最大值的索引
    top_3_indices = [item[0] for item in items[:50]]
...
def geneticAlgorithm(num, select_score, select_score2,target_score, pc, pm, generation):
    ...
    res = []
    for idx, gene in enumerate(new_popular_gene):
        gene_str = 'seq' + str(idx + 1) + ': '
...
# 随机生成序列所需时间较长
# target_score为目标序列的
res = geneticAlgorithm(num=30000,
                       select_score=1,
                       select_score2=50,
                       target_score=25,
                       pc=0.1,
